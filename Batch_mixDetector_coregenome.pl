...
$num_args = $#ARGV + 1;
if ($num_args != 2) {
   print "\n Verwendung: Batch_mixDetector.pl /Pfad/zur/Ausgabe/Dateiname.csv /Ausgabe/Pfad\n";
    exit;
}
open(Fout,">>$ARGV[0]") or die "\n\ncannot open $ARGV[0]\n\n\n";    #Ausgabedatei erstellen/Öffnen falls schon vorhanden
while ($file1=<*.bam>){
$file1=~/^(.+).bam/ or die "Eingabedateiformat nicht korrekt: $file1\n";
$bamfile=$1;
print "\n$file1\n$bamfile\n\n";

$out=$ARGV[1];#Ausgabepfad aus Übergabeparametern verwenden
...
$dir=$out;
$h37='/auto/Thecus_Analysis/SeqWork/DATA/M._tuberculosis_H37Rv_2013-02-15'; #Pfad zur Referenzdatei mit Index-Files etc


#Varianten für das Kerngenom detektieren aus der eingelesenen BAM-Datei; Pfad zum GenomeAnalysis Toolkit muss angepasst werden
...
system($do); if ($?){die "$do did not work: $?\n";}
 ...
 #--> Varianten in einen Tabelle konvertieren für R; Pfad zum GenomeAnalysisToolkit muss angepasst werden
...
system($do); if ($?){die "$do did not work: $?\n";}
 ...
 #BaseCounts ebenfalls als eigene Spalten machen und " " als seperator verwenden --> mehr Spalten als Überschriften
...
rm ${dir}/${bamfile}_Varianten2.table";  #zweimal um eventuell splitMultiAllelic hübsch zu formatieren
print "\n\n\n".$do."\n\n";
system($do); if ($?){die "$do did not work: $?\n";}
...
$R->startR ;
$R->send('library("fpc", lib.loc="/usr/lib64/R/library")');#R-Library für DBSCAN
print "Tabelle einlesen...\n";
$R->run(qq'mix<-read.table(file="${dir}/${bamfile}_Varianten.table", sep=" ", header=F, skip=1);')#Tabelle mit Basecalls einlesen und Überschriften auslassen, da weniger Überschriften als Spalten
...
dev.off()');#DBSCAN Clusterzentren berechnen und alles plotten
print "\nMixture detected: ";
$R->run('if (max(db$cluster)>1){ismix<-"YES"
anteil<-center[,1]} else if(max(db$cluster)==1 && colMeans(m[db$cluster==1,])< 0.9 ){ismix<-"YES"
anteil<-center[,1]} else {ismix<-"NO"
anteil<-center[,1]}');
$ismix=$R->get('ismix');
$anteil=$R->get('anteil');
@anteil2= grep {$_ ne 'NaN'} @$anteil;
@anteil_sort=sort(NummernSort @anteil2);
@anteil_sorted=reverse(@anteil_sort);
print "$ismix\n\nCluster center: @anteil_sorted\n";



  print Fout "$bamfile\t$ismix\t@anteil_sorted\n";  #in Ausgabetabelle schreiben
